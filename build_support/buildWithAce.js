/**
 * Inject move.js into ACE build process to package into mode-move.js .
 */
...
function build(aceVersion) {
  ...
  project.run('node tool/add_mode.js Move "sol"', 'build_cache/ace');
  project.run('cp lib/ace/mode/move* build_cache/ace/lib/ace/mode/');
  ...
  for (variety of ['src', 'src-noconflict', 'src-min', 'src-min-noconflict']) {
    ...
    project.run(`cp ./build_cache/ace/build/${variety}/mode-move.js ./build/${aceVersion}/${variety}/`);
...
function buildWithAce() {
  ...
  console.log('ACE versions to build mode-move.js for:', aceVersions.join(', '));
